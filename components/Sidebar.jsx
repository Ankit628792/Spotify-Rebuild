import { HeartIcon, HomeIcon, LibraryIcon, MenuAlt2Icon, MenuAlt3Icon, MenuAlt4Icon, PlusCircleIcon, RssIcon, SearchIcon } from '@heroicons/react/outline'
...
function Sidebar() {
    ...
    return (
        <>
            <div className={`text-gray-400 p-5 text-sm lg:text-base sm:max-w-[12rem] lg:max-w-[16rem] border-r border-gray-900 bg-black overflow-y-scroll h-screen pb-36 fixed top-0 ${isOpen ? 'left-0' : 'left-[-12rem]'} md:left-0 md:relative md:inline-flex transition-all duration-500 ease-in-out`} onClick={() => setIsOpen(false)}>
            ...
            {!isOpen && <div className='absolute top-5 left-5 rounded-full cursor-pointer md:hidden'>
                <MenuAlt2Icon className='w-10 h-10 p-[5px] rounded-full bg-black text-white' onClick={() => setIsOpen(true)} />
            </div>}
